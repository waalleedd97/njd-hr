...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {/* Identity & Personal Details — always shown */}
      <div className="glass-card rounded-2xl p-6">
        <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {([
            { icon: Hash, label: t.profile.nationalId, value: emp.nationalId, dir: "ltr" as const },
            { icon: Globe, label: t.profile.nationality, value: emp.nationality ? (emp.nationality === "saudi" ? t.profile.saudi : emp.nationality === "non-saudi" ? t.profile.nonSaudi : emp.nationality) : null },
            { icon: Cake, label: t.profile.dateOfBirth, value: emp.dateOfBirth },
            { icon: Heart, label: t.profile.maritalStatus, value: emp.maritalStatus ? (t.profile[emp.maritalStatus as keyof typeof t.profile] ?? emp.maritalStatus) : null },
            { icon: Phone, label: t.profile.mobileNumber, value: emp.mobileNumber, dir: "ltr" as const },
          ]).map((item) => (
            <div key={item.label} className="flex items-start gap-3">
              <item.icon className="w-4 h-4 text-muted-foreground mt-0.5 shrink-0" />
              <div>
                <p className="text-xs text-muted-foreground">{item.label}</p>
                <p className={cn("text-sm", item.value ? "font-medium" : "text-muted-foreground")} dir={item.dir}>
                  {item.value || "—"}
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className={cn("text-sm", emp.bankName ? "font-medium" : "text-muted-foreground")}>{emp.bankName || "—"}</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className={cn("text-sm", emp.iban ? "font-medium font-mono" : "text-muted-foreground")} dir="ltr">{emp.iban || "—"}</p>
            </div>
          </div>
        </div>
      </div>
